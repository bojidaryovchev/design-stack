...
Since 4.3.0 the detector, hook, context loader and live server are one self-contained Rust
binary, invoked through `skills/impeccable/scripts/impeccable <verb>`. It needs no Node and no
npm packages; the launcher downloads the pinned engine once and verifies it before running it.
...
| static-HTML | `.html`, with `<style>` or linked CSS | cascade-resolved rules: `oversized-h1`, `extreme-negative-tracking`, `cream-palette`, `cramped-padding`, `icon-tile-stack`, `italic-serif-display`, `hero-eyebrow-chip`, `dark-glow`, `broken-image`, `clipped-overflow-container` |
| browser + visual | **a rendered URL only** | structural and computed rules: `nested-cards`, `tiny-text`, `low-contrast`, `text-occlusion` |

Two things to know:

**The per-edit hook surfaces only a 14-rule immediate tier.** Everything else defers to the
`Stop` deep pass. Several of those 14 (`low-contrast`, `tiny-text`) are browser-tier rules that
cannot fire from a file edit at all, and four more are design-system rules that need a
`DESIGN.md` to compare against.

**A silent hook is not a clean one.** The engine binary is fetched on first use, so on a machine
with no network or no writable cache every hook fails quietly. The SessionStart hook warns when
the engine does not answer its handshake, and `npm run preflight` reports the same on demand.
Never read an absent finding as a pass without checking that first.
...
`<skill-base-dir>/scripts/impeccable detect http://localhost:3000`
...
was checked against impeccable's 62 rule IDs for non-duplication.
...
- Do not edit files under `skills/impeccable/` or `skills/emil-design-eng/` -
